...
| **B3** ⬆⬆ | **Matchup branches** (archetype detection only if needed) | **PROMOTED TO #1 2026-07-30 — it is now the repair for a MEASURED defect, not a speculation: `chip_target` scores −0.126 against `rule:crustle` while paying +0.077 in the mirror** (EVIDENCE §8c). Also the report's opponent-modeling chapter. ⚠ **Try the classifier-free version first:** the branch condition can be read straight off the board — "our attack would deal 0 to their Active" — so no archetype model is needed for the Crustle case (HANDOFF §3.3) | (a) one-line version: disable `chip_target` when their Active is undamageable, A/B n=2000 vs all three anchors; (b) only if that wins, build the ranker; (c) only if *that* needs it, build the classifier | (a) fails to beat plain `bc` on the Crustle anchor **and** does not hold the mirror → the −0.126 is not recoverable by branching, and the honest write-up is "the rule is meta-dependent, we shipped it anyway with the trade documented" |
...
| **07-30 night → 07-31** (day 9) | 🟢 **B1 LANDED — THE BREAKTHROUGH.** `optfeat` v3 (per-option target features) beats the **shipped** agent **0.661 [0.640, 0.681] n=2000** in the mirror (**≈ +115 Elo — the first effect larger than the LB's resolution**) and **0.770 vs `rule:crustle`** where shipped reads 0.663. Clean feature effect (same corpus/rows/seed): **0.878**, the largest ever measured here. **And it inverts the method: with v3 features the hand rules are HARMFUL, 0.427** — alternatives, not complements. Pool checked: min 599.7s of 600. `EVIDENCE` §8f | the report's thesis is now *"hand arithmetic is a proxy for a missing representation, and is dominated by fixing the representation"* — §8f is chapter 4 | ⏭ **package + submit the v3 agent (rules OFF)** — the first candidate clearing the "beat 950" bar; then re-mine 07-30, then `STRATEGY.md` |
