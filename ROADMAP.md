...
| **07-31** (day 9, in progress) | ❌ **Morgrem out CLOSED BY SIZING — not built** (~0.2 firings/game, the free route already 95.4% right, and a tradeoff; `EVIDENCE` §8e). Bought **rule 14 — size before you build** and a correction: "our attacker deals 0 into theirs" is true of their **Active only** (the bench snipe kills Dwebble unprevented). ⏭ next: **pilot for `crispin_toolbox`** (the #1 LB deck — no `bc`-piloted anchors); re-mine 07-30; **then B1** | ⏭ matchup win-rate matrix among high-rated players; `STRATEGY.md` skeleton | the sizing gate worked as designed — a candidate died for the price of one probe, no A/B spent |
| **08-01** | **pilot for `crispin_toolbox`**; re-mine; **B1 retrain + B4 branching probe** | matchup win-rate matrix; start `STRATEGY.md` | a second measured improvement in hand → a bundle worth submitting |
